...
class ConditionAssessorAdvanced:
    ...
    FRESHNESS_RANGES = {
        ...
        'red_produce': {
            'hue_low': 0, 'hue_high': 15,
            'saturation_low': 100, 'saturation_high': 255,
            'value_low': 50, 'value_high': 255,
        },
        # Yellow/orange produce (citrus, corn)
        'yellow_produce': {
            'hue_low': 20, 'hue_high': 35,
            'saturation_low': 100, 'saturation_high': 255,
            'value_low': 100, 'value_high': 255,
        },
    }
    ...
    def assess(self, image: np.ndarray, product_type: str = None) -> Dict[str, Any]:
        ...
        overall_score = int(
            scores['damage_score'] * 0.4 +
            scores['freshness_score'] * 0.35 +
            scores['texture_score'] * 0.25
        )
    ...
    def _analyze_freshness(self, hsv: np.ndarray, product_type: str) -> Dict[str, Any]:
        ...
        range_key = 'green_vegetable' if 'green' in product_type.lower() else \
                    'red_produce' if 'red' in product_type.lower() else \
                    'yellow_produce' if 'yellow' in product_type.lower() else \
                    'green_vegetable'  # Default
        ...
        lower = np.array([
            expected['hue_low'],
            expected['saturation_low'],
            expected['value_low']
        ], dtype=np.uint8)
        upper = np.array([
            expected['hue_high'],
            expected['saturation_high'],
            expected['value_high']
        ], dtype=np.uint8)
